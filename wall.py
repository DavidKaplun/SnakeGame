    ...
    match wall_type:
        case "up":
            ...
            for z in range(4):#there has to be a constant called wall length
                option_to_expand = gen_new_block_position_options(cur_point.get_pos_x(), cur_point.get_pos_y(), "left")#should get the block not its x and y
                new_block = chose(option_to_expand)
                if new_block!=INVALID_BLOCK:
                    wall.append(new_block)
                    cur_point.set_pos_x(new_block.get_pos_x())
                    cur_point.set_pos_y(new_block.get_pos_y())
                else:
                    break
        ...
        case "down":
            cur_point = block(apple_x - 2 * SQUARE_SIZE, apple_y + 2 * SQUARE_SIZE)
            for z in range(4):  # there has to be a constant called wall length
                option_to_expand = gen_new_block_position_options(cur_point.get_pos_x(), cur_point.get_pos_y(),"right")  # should get the block not its x and y
                new_block = chose(option_to_expand)
                if new_block != INVALID_BLOCK:
                    wall.append(new_block)
                    cur_point.set_pos_x(new_block.get_pos_x())
        ...
        case "left":
            cur_point = block(apple_x - 2 * SQUARE_SIZE, apple_y + 2 * SQUARE_SIZE)
            for z in range(4):  # there has to be a constant called wall length
                option_to_expand = gen_new_block_position_options(cur_point.get_pos_x(), cur_point.get_pos_y(), "up")  # should get the block not its x and y
                new_block = chose(option_to_expand)
                if new_block != INVALID_BLOCK:
                    wall.append(new_block)
                    cur_point.set_pos_x(new_block.get_pos_x())
        ...
        case "right":
            cur_point = block(apple_x + 2 * SQUARE_SIZE, apple_y - 2 * SQUARE_SIZE)
            for z in range(4):  # there has to be a constant called wall length
                option_to_expand = gen_new_block_position_options(cur_point.get_pos_x(), cur_point.get_pos_y(),"down")  # should get the block not its x and y
                new_block = chose(option_to_expand)
                if new_block != INVALID_BLOCK:
                    wall.append(new_block)
                    cur_point.set_pos_x(new_block.get_pos_x())
    ...
    match build_direction:
        case "up":
            for new_x in range(block_x-SQUARE_SIZE,block_x+SQUARE_SIZE):
                new_block_position_options.append(block(new_x,block_y-1))
        ...
        case "right":
            for new_y in range(block_y - SQUARE_SIZE, block_y + SQUARE_SIZE):
                new_block_position_options.append(block(block_x+1, new_y))
